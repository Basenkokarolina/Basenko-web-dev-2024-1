...
export function initializeForm() {
    ...
    everyDishes.forEach(everyDish => {
        const dish = window.dishes.find(d => d.keyword === everyDish.getAttribute("data-dish"));
...
async function submitForm() {
    ...
    try {
        const response = await fetch(`https://edu.std-900.ist.mospolytech.ru/labs/api/orders${key}`, {
            method: "POST",
            body: formData
        });
