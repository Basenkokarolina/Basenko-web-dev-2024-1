"use strict";
import {initializeForm, recalculateTotal, 
    validateLunchOrder} from "./new_order_form.js";
...
function displayMenu() {
    ...
    document.querySelectorAll(".delete-btn").forEach(btn => {
        btn.addEventListener("click", (e) => {
            const categories = ["soup", "main-course", 
                "salad", "drink", "dessert"];
            categories.forEach(category => {
                if (window.localStorage.getItem(category) === 
                e.target.closest(".every-dish").getAttribute("data-id")) {
